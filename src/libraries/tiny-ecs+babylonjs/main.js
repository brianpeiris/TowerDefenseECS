  ...
  update() {
    ...
    for (let i = 0; i < this.query.length; i++) {
      ...
      for (let j = i + 1; j < this.query.length; j++) {
        ...
        if (true || !e1c.offsetCollider.intersectsBox(e2c.offsetCollider)) continue;
        e1c.collided = e2;
        e2c.collided = e1;
      }
    }
...
class PlacementSystem {
  ...
  updatePlacement() {
    ...
    let x, z;
    const intersection = null;//scene.getIntersection();
    if (intersection) {
  ...
  update(delta) {
    for (const entity of this.query) {
      ...
      if (entity.turret.timeUntilFire <= 0) {
        ...
        projectile.mesh.mesh.position.copyFrom(entity.mesh.mesh.absolutePosition);
...
function createTurretVehicle() {
  ...
  entity.mesh.mesh.addChild(turret.mesh.mesh);
